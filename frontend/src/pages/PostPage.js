...
const PostPage = () => {
  ...
  if (loading) return <p className="page-loading">♟ Loading position...</p>;
  if (error)   return <p className="error-msg" style={{ margin: '40px 30px' }}>{error}</p>;
  if (!post)   return <p style={{ padding: '40px 30px' }}>Post not found.</p>;
      ...
      <div className="comments-section">
        ...
        {user ? (
          <form onSubmit={handleAddComment} className="comment-form">
            <textarea
              placeholder="Share your thoughts on this position..."
              value={commentBody}
